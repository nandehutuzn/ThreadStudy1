...
namespace 调整PLINQ查询参数
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                parallelQuery.WithDegreeOfParallelism(Environment.ProcessorCount).WithExecutionMode
                    (ParallelExecutionMode.ForceParallelism).WithMergeOptions(ParallelMergeOptions.Default)
            ...
            var unorderQuery = from i in ParallelEnumerable.Range(1, 30) select i;
